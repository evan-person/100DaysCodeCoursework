...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for transition, parse_sentence in zip(transitions, minibatch):
            parse_sentence.parse_step(transition)
            if len(parse_sentence.buffer) == 0 and len(parse_sentence.stack) == 1:
                unfinished_parses.remove(parse_sentence)
